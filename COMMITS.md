...
- Implemented soft deletes for Forum Topics via the \hidden\ field (\schema.prisma\, \types/forum.ts\).
...
- Refactored layout configurations and map keys within \app/profile/settings/layout.tsx\ and \
  navbar.tsx\.
- Adjusted Bootstrap modal utility integrations inside \loginModal.tsx\.
- Added image domain \mc-heads.net\ to \
  next.config.js\ and updated image avatars.
...
# 05-12-2026 - Forum Creation Flow Fixes & Editor Documentation

- **Bug Fix**: Addressed React dependency loop warnings in useForum.ts by wrapping showLoadingBar, hideLoadingBar in a useCallback inside 	heme.ts.
- **Bug Fix**: Repaired PrismaClientKnownRequestError on Subcategories during createTopic by fixing schema structures and properly routing the IDs.
- **Bug Fix**: Fixed unhandled Next.js NEXT_REDIRECT errors during topic creation success callbacks by replacing server-side edirect() with useRouter().push().
- **Documentation**: Augmented DOC.md with editor.tsx usage documentation and updated FILES.md to reflect the latest project structure.
- **Feature**: Added missing Select.tsx and modified TextArea.tsx for usage in 	opicCreationForm.tsx.
- **Cleanup**: Moved /topic/[id]/add to /topic/add and /topic/new.

M COMMITS.md
M DOC.md
M FILES.md
D src/app/forum/topic/[id]/add/page.tsx
A src/app/forum/topic/add/page.tsx
