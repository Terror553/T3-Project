...
46. Dedicated dashboard policy and registration settings

- Replaced the privacy/terms and registration placeholders with Bootstrap forms
  backed by the existing authorized configuration API. Staff can edit policy
  text, enable or disable registration, and require email verification with
  visible loading and save feedback. Store implementation remains intentionally
  out of scope.
- Validation: `npm run check` passed.
- Key files:
  - C:/Users/win11/Desktop/Backup/T3-Project.worktrees/todo-list-prioritization-plan/src/app/dashboard/configuration/privacy/page.tsx
...
50. Public policy page persistence

- Replace the public rules placeholder with persisted privacy-policy and terms
  content from dashboard configuration, retaining the Bootstrap tab/card
  presentation and safe empty-state messaging.
...
52. Dashboard-native reaction management

- Replace the dashboard reaction route re-export with a dashboard section that
  uses the existing authorized reaction API, providing Bootstrap-native create,
  list, delete, loading, empty, and error states without duplicating backend
  behavior.
- Validation: `npm run check` passed. No dedicated reaction test file exists in
  the repository.
- Key files:
...
36. Dashboard configuration API coverage

- Added focused tests for staff authorization, default configuration values,
  invalid partial updates, and persisted configuration updates.
...
38. Forum reports API coverage

- Added focused tests for authenticated report submission, exactly-one-target
  validation, staff authorization, and moderation status updates.
- Validation: the focused Vitest suite and `npm run check` passed.
- Key file:
...
42. Forum page-family visual parity

- Updated the forum index and category/subcategory components to use the
  audited page header, search control, forum card, responsive subcategory row,
  accessible collapse label, and explicit loading/error/empty state patterns.
- Validation: `npm run check` passed; focused forum tests passed (18 tests).
- Key files:
  - C:/Users/win11/Desktop/Backup/T3-Project.worktrees/todo-list-prioritization-plan/src/app/forum/page.tsx
  - C:/Users/win11/Desktop/Backup/T3-Project.worktrees/todo-list-prioritization-plan/src/components/forum/ForumCategoryItem.tsx
  - C:/Users/win11/Desktop/Backup/T3-Project.worktrees/todo-list-prioritization-plan/src/components/forum/ForumSubcategoryItem.tsx

43. Profile and member page-family visual parity

- Updated the members directory and public profile loading/not-found states
  with the audited content, page-header, card, badge, and alert patterns.
  Corrected the reusable profile utility bar's account links and labels.
- Validation: `npm run check` passed.
- Key files:
  ...
  - C:/Users/win11/Desktop/Backup/T3-Project.worktrees/todo-list-prioritization-plan/src/components/profileBar.tsx

44. Authentication page-family visual parity

- Updated login and registration surfaces with audited page headers, card
  headers, labeled controls, Bootstrap action buttons, and accessible alerts.
  Corrected the registration success condition encountered in the same flow.
- Validation: `npm run check` passed.
- Key files:
